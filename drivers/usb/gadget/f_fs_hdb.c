...
#define INIT_COMPLETION_HDB(x)     ((x).done = 0)
...
static int __hdb_ffs_ep0_queue_wait(struct hdb_ffs_data *hdb_ffs, char *data, size_t len)
{
	...
	INIT_COMPLETION_HDB(hdb_ffs->ep0req_completion);
...
first_try:
	...
	if (unlikely(halt)) {
		...
		if (read) {
			INIT_COMPLETION_HDB(hdb_ffs->epout_completion);
			req->context  = done = &hdb_ffs->epout_completion;
		} else {
			INIT_COMPLETION_HDB(hdb_ffs->epin_completion);
...
hdb_ffs_fs_kill_sb(struct super_block *sb)
{
	...
	if (sb->s_fs_info) {
		functionfs_hdb_release_dev_callback(sb->s_fs_info);
