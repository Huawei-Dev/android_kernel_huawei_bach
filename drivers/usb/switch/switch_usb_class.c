...
static struct switch_usb_info *p_switch_usb_info;
...
void usb_custom_acc5_event(int pedestal_attach)
{
	...
	if (pedestal_attach_flag == 1) {
		snprintf(event_buf, sizeof(event_buf),
				"USB_CUSTOM_ACC5=pedestal_attach");
	} else {
		snprintf(event_buf, sizeof(event_buf),
				"USB_CUSTOM_ACC5=pedestal_detach");
	}
	if (NULL == sdev.dev) {
		hwlog_info("%s: sdev.dev is NULL, do nothing!\n",__func__);
		return;
	}
...
int switch_usb_register_notifier(struct notifier_block *nb)
{
	...
	if (p_switch_usb_info != NULL) {
		...
		ret = atomic_notifier_chain_register(
				&p_switch_usb_info->charger_type_notifier_head,
				nb);
		spin_unlock_irqrestore(&p_switch_usb_info->reg_flag_lock,
					flags);
	}
			      ...
			      char *buf)
{
	struct switch_usb_dev *sdev =
				(struct switch_usb_dev *)dev_get_drvdata(dev);
...
static int switch_usb_dev_register(struct switch_usb_dev *sdev)
{
	...
	if (IS_ERR_OR_NULL(switch_usb_class)) {
		hwlog_info("%s: switch_usb_class is ERR or NULL, and create_switch_usb_class will start.",
				__func__);
	...
	sdev->dev = device_create(switch_usb_class, NULL, MKDEV(0, 0),
					NULL, "usbsw");
	if (IS_ERR(sdev->dev)) {
...
static int usb_switch_probe(struct platform_device *pdev)
{
	...
	if (gpio_switch_back < 0) {
		hwlog_err("%s: of_get_named_gpio error!!! gpio_switch_back=%d",
				__func__, gpio_switch_back);
		/* invalid value */
		sdev.irq1 = -1;
	} else {
