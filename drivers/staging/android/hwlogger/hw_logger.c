...
static long logger_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FIONREAD:
		if (!strncmp(log->misc.name, LOGGER_LOG_JANK, strlen(LOGGER_LOG_JANK)) ||
			!strncmp(log->misc.name, LOGGER_LOG_PTM, strlen(LOGGER_LOG_PTM)) ||
			!strncmp(log->misc.name, LOGGER_LOG_BDAT,strlen(LOGGER_LOG_BDAT))) {
			ret = -ENOTTY;
		}
		break;
	}
...
static int __init logger_init(void)
{
	...
	if (unlikely(ret))
		goto out;
	ret = create_log(LOGGER_LOG_BDAT, 64 * 1024);
	if (unlikely(ret))
		goto out;
	ret = create_log(LOGGER_LOG_PTM, 16 * 1024);
	if (unlikely(ret))
		goto out;
#endif
