...
static ssize_t store_virt_addr(struct kobject *dev,
...
static struct kobj_attribute kvirt_attr =
		__ATTR(virt_to_phys, 0640, show_phys_addr, store_virt_addr);

static int __init virt_to_phys_init(void)
{
	int error;

	error = sysfs_create_file(kernel_kobj, &kvirt_attr.attr);
	if (error) {
...
{
	sysfs_remove_file(kernel_kobj, &kvirt_attr.attr);
}
