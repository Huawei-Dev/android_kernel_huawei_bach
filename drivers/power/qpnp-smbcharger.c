...
#define IN_THERMAL_ICL_VOTER	"IN_THERMAL_ICL_VOTER"
...
#define HVDCP_HW_VOTER			"HVDCP_HW_VOTER"
...
static int get_prop_batt_charge_full_design(struct smbchg_chip *chip)
{
	...
	if (!chip) {
		pr_smb(PR_STATUS, "Invalid param\n");
		return 0;
	}
...
out:
	chip->hvdcp_3_det_ignore_uv = false;
	if (rc && !is_src_detect_high(chip)) {
		pr_smb(PR_MISC, "DCP force removed\n");
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		rc = vote(chip->usb_icl_votable, USER_ICL_VOTER, true,
					val->intval);
		if (rc < 0)
			pr_err("Couldn't update USB USER ICL vote rc=%d\n", rc);
		break;
	default:
		return -EINVAL;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_PRESENT:
		val->intval = !!(is_usb_present(chip) | is_dc_present(chip) |
						chip->hvdcp_3_det_ignore_uv);
		break;
	default:
		return -EINVAL;
...
static int determine_initial_status(struct smbchg_chip *chip)
{
    ...
    if (chip->device_not_support_hvdcp) {
        vote(chip->hvdcp_enable_votable, HVDCP_HW_VOTER, true, 0);
    }
	if (chip->typec_psy) {
		get_property_from_typec(chip, POWER_SUPPLY_PROP_TYPE, &type);
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	OF_PROP_READ(chip,dcp_icl_ma,"dcp_icl_ma",rc,1);
	if(dcp_icl_ma > 0){
		pr_info("dcp_icl_ma = %d\n",dcp_icl_ma);
		smbchg_default_dcp_icl_ma = dcp_icl_ma;
	}
	...
	OF_PROP_READ(chip, chip->charger_batt_capacity_mah,
			"charger-batt-capacity-mah", rc, 1);

	/* read boolean configuration properties */
	chip->ignore_usb_ibus_setting = of_property_read_bool(node,
...
static int qpnp_smb_set_in_thermal(int val)
{
	...
	current_limit = min(chip->current_adjusted_ma, hot_design_current);
