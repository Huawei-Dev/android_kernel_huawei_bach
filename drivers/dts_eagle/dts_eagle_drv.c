...
{
    /*
     * The parameter f does been used now. Now we add some Log info to avoid PClint Warning.
     */
    if (f != NULL) {
       dts_eagle_drv_dbg_msg("%s , fPtr: %p", __func__, f);
    }else {
...
static long dts_eagle_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	down(&lock);
         ...
         */
        if (f == NULL) {
            dts_eagle_drv_err_msg("%s: error invalid file",
                                 __func__);
            up(&lock);
            return -EINVAL;
        }
