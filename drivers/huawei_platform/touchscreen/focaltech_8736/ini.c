...
//		return CFG_ERR_OPEN_FILE; 
	...
	while(1) {/*搜找项section */
		...
		if(buf1[0] == CFG_SSL) {
			if (max!=0){
				buf1[n-1] = 0x00;
				strncpy((char *)sections[n_sections], buf1+1, MAX_CFG_BUF);
				if (n_sections>=max)
					break;		/* 超过可返回最大个数 */
			}
			n_sections++;
...
//		return CFG_ERR_OPEN_FILE; 
	...
	while(1) {/*搜找项section */
		...
		if(buf1[0] == CFG_SSL) {	
			...
			buf1[n-1] = 0x00;
			strncpy((char *)tmpSectionName, buf1+1, MAX_CFG_BUF);
			//printk("Section Name:%s, Len:%d\n\n", tmpSectionName, n-2);
		...
		strncpy( g_st_ini_file_data[g_used_key_num].pSectionName, tmpSectionName, MAX_KEY_NAME_LEN-1);
