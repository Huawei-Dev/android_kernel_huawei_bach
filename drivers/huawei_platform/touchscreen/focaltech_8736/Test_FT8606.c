...
#define MAX_TMPBUFF_SIZE	(1024*16)
#define MAX_STORE_AREA_SIZE	(1024*80)
/////////////////////////////////////////////////Reg 8606
...
static char g_pStoreAllData[MAX_STORE_AREA_SIZE] = {0};
...
static void AllocateMemory(void)
{
	...
	if(NULL == g_pStoreMsgArea)
		g_pStoreMsgArea = kmalloc(MAX_STORE_AREA_SIZE, GFP_ATOMIC);
	g_pMsgAreaLine2 =NULL;	
	if(NULL == g_pMsgAreaLine2)
		g_pMsgAreaLine2 = kmalloc(MAX_STORE_AREA_SIZE, GFP_ATOMIC);
	g_pStoreDataArea =NULL;	
	if(NULL == g_pStoreDataArea)
		g_pStoreDataArea = kmalloc(MAX_STORE_AREA_SIZE, GFP_ATOMIC);
	/*g_pStoreAllData =NULL;	
	if(NULL == g_pStoreAllData)
	g_pStoreAllData = kmalloc(1024*8, GFP_ATOMIC);*/
	g_pTmpBuff =NULL;	
	if(NULL == g_pTmpBuff)
		g_pTmpBuff = kmalloc(MAX_TMPBUFF_SIZE, GFP_ATOMIC);
...
static void InitStoreParamOfTestData(void)
{
	g_lenStoreMsgArea = 0;
	//Msg Area, Add Line1
	g_lenStoreMsgArea += snprintf(g_pStoreMsgArea, MAX_STORE_AREA_SIZE, "ECC, 85, 170, IC Name, %s, IC Code, %x\n", g_strIcName,  g_ScreenSetParam.iSelectedIC);
...
unsigned char FT8606_TestItem_NoiseTest(bool* bTestResult)
{
	...
	//Get RawData
	memset(m_TempNoiseData, 0, MAX_NOISE_FRAMES * RX_NUM_MAX * TX_NUM_MAX);
	memset(m_NoiseData, 0, TX_NUM_MAX * RX_NUM_MAX);
	...
	else if(NT_MaxDevication == g_stCfg_FT8606_BasicThreshold.NoiseTest_NoiseMode)
	{
		...
		memset(m_iTempData, 0xffff, TX_NUM_MAX * RX_NUM_MAX);//Save The Min Value	
		memset(m_NoiseData, 0, TX_NUM_MAX * RX_NUM_MAX);	//Save The Max Value
		for(i = 0; i < iNoiseFrames; i++)
		{
			for(iRow = 0; iRow < g_stSCapConfEx.ChannelXNum + 1; iRow++)
			{
...
unsigned char FT8606_TestItem_CbTest(bool* bTestResult)
{
	...
	memset(m_CBData, 0, TX_NUM_MAX * RX_NUM_MAX);
	///VA area
	for ( iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; ++iRow )
	{
		for ( iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; ++iCol )
