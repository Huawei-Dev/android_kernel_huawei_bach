...
#define HWLOG_BDAT_FS "hwlog_bdat"
HWLOG_REGIST();
...
static const char *log_name[HW_LOG_ID_MAX] = {
	...
	[HW_LOG_ID_BDAT]   = "/dev/" HWLOG_BDAT_FS,
};
/**
...
static int __write_to_kernel_kernel(struct my_work_struct  *phwlog)
{
	...
	if (HW_LOG_ID_JANK == phwlog->nbufid) {
		vec[0].iov_base = &phwlog->nPrio;
		vec[0].iov_len  = 2;
		vec[1].iov_base = &phwlog->janktagid;
		vec[1].iov_len  = 2;
		vec[2].iov_base =&phwlog->uptime;
		vec[2].iov_len  = sizeof(phwlog->uptime);
		vec[3].iov_base = &phwlog->realtime;
		vec[3].iov_len  = sizeof(phwlog->realtime);
		vec[4].iov_base = phwlog->msg;
		vec[4].iov_len  = phwlog->msglen;
		vlen = 5;
	} else {
		vec[0].iov_base = &phwlog->nPrio;
		vec[0].iov_len  = 1;
		vec[1].iov_base = phwlog->ntagid;
		vec[1].iov_len  = phwlog->taglen;
		vec[2].iov_base = phwlog->msg;
		vec[2].iov_len  = phwlog->msglen;
		vlen = 3;
	}
	oldfs = get_fs();
	set_fs(get_ds());
