...
extern int mmc_sd_runtime_resume(struct mmc_host *host);

static ssize_t mmc_lockable_show(struct device *dev,
...
static int mmc_lock_sd_init_card(struct mmc_card *card, bool binit)
{
	...
	if (true == card->swith_voltage) {
		...
		/*mmc_sd_card_set_uhs(card);//temp remove this*/
...
static int sd_erase_reinit(struct mmc_card *card , struct device *dev)
{
	...
	mmc_delay(200);
	/*mmc_power_up(card->host, card->host->ocr);*/
...
static int sd_resume_clk_scaling(struct mmc_host *host)
{
	int err = 0;
	u32 max_clk_idx = 0;
...
static int resume_clk_scaling(struct mmc_card* card)
{
	int ret = 0;
	int retries = 0;
	...
	retries = 5;
	while(retries){
		ret = mmc_sd_init_card(card->host, card->ocr, card);
		if(ret){
			printk("[SDLOCK]:%s: mmc_sd_init_card fail\n", __func__);
			retries--;
			mmc_power_off(card->host);
			usleep_range(5000, 5500);
			mmc_power_up(card->host, card->ocr);
			mmc_select_voltage(card->host, card->ocr);
			continue;
		}
		break;
	}

	mmc_card_clr_suspended(card);
	ret = sd_resume_clk_scaling(card->host);
	if(ret){
		printk("[SDLOCK]:%s: fail to resume clock scaling\n", __func__);
		ret = -EINVAL;
	}
		...
		const char *data, size_t len)
{
	...
	if (mmc_bus_needs_resume(card->host)) {
		printk("[SDLOCK] mmc_bus_needs_resume is true \r\n");
	} else {
		printk("[SDLOCK] mmc_bus_needs_resume is false \r\n");
		mmc_sd_runtime_resume(card->host);
	}
#else
	{
		mmc_sd_runtime_resume(card->host);
	...
	if (mmc_card_locked(card) && !strncmp(command, "erase", 5)) {
		...
		ret = resume_clk_scaling(card);
		if(ret){
			printk("%s: resume clk fail (%d)\n", __func__, ret);
		}
