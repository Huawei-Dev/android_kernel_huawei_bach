...
static int ts_detect_ghost_algo2(int index) {
	...
	if ((delta_x < GHOST_OPER_DISTANCE_ALGO2) && (delta_x > -1*GHOST_OPER_DISTANCE_ALGO2)){
		ghost_num_record_x[index] ++;
	}else{
		ghost_num_record_x[index] = 0;
	}
	if((delta_y < GHOST_OPER_DISTANCE_ALGO2) && (delta_y > -1*GHOST_OPER_DISTANCE_ALGO2)){
			...
			if(delta_time < FINGER_PRESS_TIME_MIN_ALGO3) {
				for(i=0;i<GHOST_OPERATE_TWO_FINGERS;i++)
					for(j=0;j<GHOST_OPERATE_TWO_FINGERS;j++){
						...
						if ((delta_x < GHOST_OPER_DISTANCE_ALGO3) && (delta_x > -1*GHOST_OPER_DISTANCE_ALGO3)
							&&(delta_y < GHOST_OPER_DISTANCE_ALGO3) && (delta_y > -1*GHOST_OPER_DISTANCE_ALGO3) ) {
				...
				delta_time = (finger_touch.finger_press_tv[1].tv_sec - finger_touch.finger_press_tv[0].tv_sec)*GHOST_MIL_SECOND_TIME
						+ finger_touch.finger_press_tv[1].tv_sec -finger_touch.finger_press_tv[0].tv_sec;
				if ((delta_x_0 < GHOST_OPER_DISTANCE_ALGO3) && (delta_x_0 > -1*GHOST_OPER_DISTANCE_ALGO3)&&(delta_time < FINGER_PRESS_TIME_MIN_ALGO3)
					&&(delta_y_0 < GHOST_OPER_DISTANCE_ALGO3) && (delta_y_0 > -1*GHOST_OPER_DISTANCE_ALGO3) ) {
					ghost_algo3_num++;
				}else if((delta_x_1 < GHOST_OPER_DISTANCE_ALGO3) && (delta_x_1 > -1*GHOST_OPER_DISTANCE_ALGO3)&&(delta_time < FINGER_PRESS_TIME_MIN_ALGO3)
					&&(delta_y_1 < GHOST_OPER_DISTANCE_ALGO3) && (delta_y_1 > -1*GHOST_OPER_DISTANCE_ALGO3)) {
					ghost_algo3_num++;
				}else{
					ghost_algo3_num = 0;
					target_x = 0;
