	...
	for (i = 0; i < rows; i++) {
		...
		while ((token = strsep(&tok_ptr,", \t\n\r\0"))) {
			...
			if(type == MUTUAL_RAW_MAX) {
				rawdata[j] = (int32_t)simple_strtol(token, NULL, STRTOL_LEN);
			} else if (type == MUTUAL_RAW_MIN) {
				rawdata[j] = (int32_t)simple_strtol(token, NULL, STRTOL_LEN);
			} else if (type == MUTUAL_RX_DELTA ||
						type == MUTUAL_TX_DELTA) {
				rawdata[j] = (int32_t)simple_strtol(token, NULL, STRTOL_LEN);
			} else
...
int ts_kit_parse_csvfile(int columns, int rows, struct ts_rawdata_limit_tab* limit_tab, char *file_path)
{
	...
	if (read_ret > 0) {
		...
		if (limit_tab && limit_tab->MutualRawMax) {
		...
		if (limit_tab && limit_tab->MutualRawMin) {
		...
		if (limit_tab && limit_tab->unique_test &&
			SUPPORT_UNIQUE_TEST == limit_tab->unique_test->Read_only_support_unique) {
			ptr = buf;
			ptr = strstr(ptr, "Rx_delta_abslimit");
			if (ptr == NULL) {
				TS_LOG_ERR("%s: load Rx_delta_abslimit failed!\n", __func__);
				ret = UNIQUE_TEST_FAIL;
				goto exit_free;
			}
			goto_next_line(&ptr); //next row: Rx_delta_abslimit
			if ((NULL == ptr) || (0 == strlen(ptr))) {
				TS_LOG_ERR("%s: goto_next_line failed!\n", __func__);
				ret = UNIQUE_TEST_FAIL;
				goto exit_free;
			}
			if (limit_tab->unique_test->Rx_delta_abslimit) {
				parse_valid_data(buf, stat.size, ptr, MUTUAL_RX_DELTA, limit_tab->unique_test->Rx_delta_abslimit, rows);
				print_data(MUTUAL_RX_DELTA, limit_tab->unique_test->Rx_delta_abslimit, rows, columns);
			}
			ptr = buf;
			ptr = strstr(ptr, "Tx_delta_abslimit");
			if (ptr == NULL) {
				TS_LOG_ERR("%s: load Tx_delta_abslimit failed!\n", __func__);
				ret = UNIQUE_TEST_FAIL;
				goto exit_free;
			}
			goto_next_line(&ptr); //next row: Tx_delta_abslimit
			if ((NULL == ptr) || (0 == strlen(ptr))) {
				TS_LOG_ERR("%s: load goto_next_line failed!\n", __func__);
				ret = UNIQUE_TEST_FAIL;
				goto exit_free;
			}
			if (limit_tab->unique_test->Tx_delta_abslimit) {
				parse_valid_data(buf, stat.size, ptr, MUTUAL_TX_DELTA, limit_tab->unique_test->Tx_delta_abslimit, rows);
				print_data(MUTUAL_TX_DELTA, limit_tab->unique_test->Tx_delta_abslimit, rows, columns);
			}
		}
	} else {
