				     ...
				     struct ts_cmd_node *out_cmd)
{
	...
	ret = novatek_ts_kit_i2c_read(nvt_ts->client, I2C_FW_Address, point_data, 64 + 1);
	if (ret < 0) {
...
int32_t novatek_kit_read_projectid(void)
{
	...
	if(strcmp(novatek_kit_project_id, wrong_id) == 0){
		...
		strncpy(novatek_kit_product_id, "VCTO33100", PROJECT_ID_LEN);
		strncpy(novatek_kit_project_id, "VCTO33100", PROJECT_ID_LEN);
	}
	//----------------------------------------------------------
...
static void novatek_ghost_detect(int value){
	...
	if (!dsm_client_ocuppy(ts_dclient)) {
		dsm_client_record(ts_dclient, "novatek ghost touch detect.\n");
		dsm_client_notify(ts_dclient, DSM_TP_GHOST_TOUCH_ERROR_NO);
	}
#endif
}
