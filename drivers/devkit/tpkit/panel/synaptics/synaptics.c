...
#define PECULIA_TRX_DELTA_TEST "huawei,trx_delta_test_support"
	...
	if(rmi4_data->synaptics_chip_data->enable_ghost_dmd_report == 0) {
		if (!dsm_client_ocuppy(ts_dclient)) {
			dsm_client_record(ts_dclient,
					  "noise record number: noise_record_num is %d.frequency_selection is %d\n",
...
static void synaptics_check_crc_status(unsigned short ic_status_reg)
{
	...
	if (synaptics_rmi4_device_status_check(status)) {
		...
		if (!dsm_client_ocuppy(ts_dclient)) {
			...
			dsm_client_record(ts_dclient, "[synaptics]device status for 20004 is:%d\n", status.status_code);
			dsm_client_notify(ts_dclient, DSM_TP_DEV_STATUS_ERROR_NO);
		}
#endif
	}
	return;
...
{	
	...
	retval = of_property_read_u32(device, PECULIA_TRX_DELTA_TEST, &chip_data->trx_delta_test_support);
	if (retval) {
	...
	retval = of_property_read_u32(device, NO_TEST_CHANGE_RATE, &chip_data->no_change_report_rate);
	if (retval) {
		TS_LOG_INFO("not support_change_report_rate is 0.\n");
		chip_data->no_change_report_rate = 0;
		retval = 0;
	}
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type
		...
		if (rmi4_data->force_update || need_update) {
			...
			if (retval) {
				...
				strncpy(rmi4_data->synaptics_chip_data->ts_platform_data->dsm_info.fw_update_result,
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (status.flash_prog == 1) {
					...
					if (!dsm_client_ocuppy(ts_dclient)) {
						...
						dsm_client_record(ts_dclient, "device status for 20003 is :%d\n", status.status_code);
				...
				if (synaptics_rmi4_device_status_check(status)) {
					...
					if (!dsm_client_ocuppy(ts_dclient)) {
						TS_LOG_INFO ("try to client record 20004 \n");
						dsm_client_record(ts_dclient, "device status for 20004 is :%d\n",status.status_code);
						dsm_client_notify(ts_dclient,DSM_TP_DEV_STATUS_ERROR_NO);
					}
#endif
				}
