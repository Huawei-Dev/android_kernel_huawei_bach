...
#define TD43XX_EE_SHORT_PART_ONE_THRESHOLD 100
#define TD43XX_EE_SHORT_PART_TWO_THRESHOLD 90
...
static int f54_rawimage_report(void)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->raw_test_type == RAW_DATA_TEST_TYPE_SINGLE_POINT) {
		...
		if (ts_kit_parse_csvfile(columns_size, rows_size, &limit_tab, file_path)) {
...
*/
	...
	if (Result == (mmi_buf_size / 2)) {
		...
		if (!dsm_client_ocuppy(ts_dclient)) {
			...
			dsm_client_record(ts_dclient, "device status for 20002 is :%d\n", Result);
...
static int td43xx_ee_short_normalize_data(signed short * image)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (temp < TD43XX_EE_SHORT_PART_TWO_THRESHOLD) {
				image[i * rx_num + j] = 1;
			}
			else {
				image[i * rx_num + j] = 0;
...
static int td43xx_ee_short_report(void)
{
	...
	TS_LOG_INFO("%s: report_type=%d,tx=%d,rx=%d #%d\n",
	...
	for (i = 0, k = 0; i < tx_num * rx_num; i++) {
		...
		td43xx_rt95_part_one[i] = (td43xx_rt95_part_one[i] > TD43XX_EE_SHORT_PART_ONE_THRESHOLD) ? 1 :0;
		k += NORMAL_NUM_TWO;
	}
...
static int synaptics_get_threshold_from_csvfile(int columns, int rows, struct ts_rawdata_limit_tab* limit_tab)
{
	char file_path[100] = {0};
	char file_name[64] = {0};
	int ret = 0;
	...
	if (ts_kit_parse_csvfile(columns, rows, limit_tab, file_path)) {
		TS_LOG_ERR("csv file parse fail:%s\n", file_path);
		ret = TEST_FAIL;
	} else {
		ret = TEST_PASS;
...
static int f54_delta_rx2_report(void)
{
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(columns_size-1, rows_size, &limit_tab)) {
...
static int f54_delta_tx2_report(void)
{
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(columns_size, rows_size-1, &limit_tab)) {
...
static int check_csvfile_raw_capacitance(void)
{
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(columns_size, rows_size, &limit_tab)) {
		TS_LOG_ERR("get rawdata_cap_limit err\n");
		ret = TEST_FAIL;
		goto error_release_mem;
	}
...
static void synaptics_change_report_rate(void)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->no_change_report_rate) {
		TS_LOG_INFO("TDDI do not test change_report_rate\n");
		return;
	}
	...
	if ((F54_DATA120_RATE == report_rate120)
		...
		strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
#if defined (CONFIG_HUAWEI_DSM)
		if (atomic_read(&rmi4_data->synaptics_chip_data->ts_platform_data->state) != TS_WORK_IN_SLEEP) {
			if (!dsm_client_ocuppy(ts_dclient)) {
				TS_LOG_INFO ("try to client record 20006 for change report rate error \n");
				dsm_client_record(ts_dclient, "for 20006 report_rate120 is :%d, report_rate60 is : %d\n", report_rate120,report_rate60);
				dsm_client_notify(ts_dclient, DSM_TP_CHANGE_REPORT_ERROR_NO);
			}
		}
#endif
	}
