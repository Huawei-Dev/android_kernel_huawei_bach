...
static void __add_dirty_inode(struct inode *inode, enum inode_type type)
{
	...
	if (is_inode_flag_set(fi, FI_DIRTY_DIR) || is_inode_flag_set(fi, FI_DIRTY_FILE))
		return;
...
static void __remove_dirty_inode(struct inode *inode, enum inode_type type, bool force)
{
	struct f2fs_inode_info *fi = F2FS_I(inode);
	int flag = (type == DIR_INODE) ? FI_DIRTY_DIR : FI_DIRTY_FILE;

	if ((!force && get_dirty_pages(inode)) ||
...
void remove_dirty_inode(struct inode *inode, bool force)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	struct f2fs_inode_info *fi = F2FS_I(inode);
	enum inode_type type = S_ISDIR(inode->i_mode) ? DIR_INODE : FILE_INODE;
	...
	spin_lock(&sbi->inode_lock[type]);
	__remove_dirty_inode(inode, type, force);
	spin_unlock(&sbi->inode_lock[type]);
