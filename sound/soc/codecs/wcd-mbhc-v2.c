...
#define MBHC_DETECT_INSERT_DBNC_DEFAULT 6
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	if (is_bootmode_factory
		...
		if (mbhc->force_gnd_mic_swap_to_headphone) {
			pr_info("%s: force plug type to headphone\n", __func__);
			plug_type = MBHC_PLUG_TYPE_HEADPHONE;
		}
	}
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_INSREM_DBNC, mbhc->detect_insert_debounce);
		      ...
		      bool impedance_det_en)
{
	...
	int hph_swh = 0;
	int gnd_swh = 0;
	int insert_debounce = 0;
	struct snd_soc_card *card = codec->component.card;
	const char *hph_switch = "qcom,msm-mbhc-hphl-swh";
	const char *gnd_switch = "qcom,msm-mbhc-gnd-swh";
	const char *gnd_mic_swap = "qcom,force-gnd-mic-swap-to-headphone";
	const char *detect_insert_debounce = "qcom,msm-mbhc-insert-debounce";
	const char *hph_gain_high = "qcom,msm-mbhc-headphone-gain-default-high";
	...
	ret = of_property_read_u32(card->dev->of_node, detect_insert_debounce, &insert_debounce);
	if (ret) {
		dev_err(card->dev,"%s: missing %s in dt node, set to default\n", __func__, detect_insert_debounce);
		insert_debounce = MBHC_DETECT_INSERT_DBNC_DEFAULT;
	}
	mbhc->in_swch_irq_handler = false;
	mbhc->current_plug = MBHC_PLUG_TYPE_NONE;
	...
	mbhc->detect_insert_debounce = insert_debounce;
	mbhc->force_gnd_mic_swap_to_headphone =
		of_property_read_bool(card->dev->of_node, gnd_mic_swap);
	mbhc->hph_gain_high_as_default =
		of_property_read_bool(card->dev->of_node, hph_gain_high);
