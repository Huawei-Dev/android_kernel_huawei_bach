/*
 * max98925.c -- ALSA SoC Stereo/Mono MAX98925 driver
 *
...
#define MAX_ARRAY_LEN 384
#define PAYLOAD_START 100
#define MAX_DIR_WRITE 1
#define MAX_DIR_READ  0
extern int afe_dsm_param_ctrl(uint32_t dir, uint32_t size, uint8_t *payload);
static int param_array[MAX_ARRAY_LEN];
static uint8_t *payload = (uint8_t *)&param_array[PAYLOAD_START];
#endif

#define ENABLE_SPEAKER_SWAP      "enable_maxim_speaker_swap"
#define CONFIG_SPEAKER_SWITCH    "speaker_receiver_switch"
...
#define STEREO_NORMAL_MODE       (0x3)
#define STEREO_REVERSE_MODE      (0x4)
#define DEBUG_BUF_LEN 64
#define OPEN_RECV     2
#define OPEN_SPKR     1

static uint32_t orient_mode = STEREO_NORMAL_MODE;
static DEFINE_MUTEX(dsm_lock);
static struct max98925_priv *pmax98925data;
static bool pa_enable;
static bool enable_spk_swap;
static int gpio_status;
static bool g_max_drv_err = true;
static uint8_t i2c_node_info[DEBUG_BUF_LEN] = {0};
...
void reg_dump(struct max98925_priv *max98925)
{
	...
	i = 0;
	while (0 != reg_table[i].count) {
		for (j = 0; j < reg_table[i].count; j++) {
			int ret = 0;
			addr = j + reg_table[i].start;
			ret = regmap_read(max98925->regmapL, addr, &val_l);
			if (ret < 0) {
				pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
			}
			if (max98925->regmapR) {
				ret = regmap_read(max98925->regmapR, addr, &val_r);
				if (ret < 0) {
					pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
				}
			}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = regmap_read(max98925->regmapL, MAX98925_R02D_GAIN, &data);
	if (ret < 0) {
		pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
	}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (max98925->regmapR)
	{
		ret = regmap_read(max98925->regmapR, MAX98925_R02D_GAIN, &data);
		if (ret < 0) {
			pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
		}
	}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (DEFAULT_SWITCH_NONEED != max98925->switch_en_gpio && sel) {
		...
		pr_err("%s: spk&rcver switch gpio not set", __func__);
	}

	max98925->left_en = sel << M98925_EN_SHIFT;
	if (NULL == max98925->regmapR)
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (DEFAULT_SWITCH_NONEED != max98925->switch_en_gpio && sel) {
		...
		pr_err("%s: not config spk&rcver switch gpio\n", __func__);
	}
		...
		unsigned int mask, unsigned int shift)
{
	...
	ret = regmap_read(max98925->regmapL, reg, &data);
	if (ret < 0) {
		pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
	}
...
static int max98925_swap_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol, unsigned int reg,
		unsigned int mask, unsigned int shift)
{
	if (STEREO_NORMAL_MODE == orient_mode)
...
/*swap channels if rcv swap command*/
static int max98925_swap_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol, unsigned int reg,
		unsigned int mask, unsigned int shift)
{
	unsigned int sel = ucontrol->value.integer.value[0];
	...
	orient_mode = (sel) ? STEREO_REVERSE_MODE : STEREO_NORMAL_MODE;
	param[0] = orient_mode;
	param[1] = 0;
	afe_dsm_param_ctrl(MAX_DIR_WRITE, 16, (uint8_t *)param);
	if (sel)
		pr_info("send command for speaker reverse\n");

	return 0;
}

static int max98925_swap_channels_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return max98925_swap_get(kcontrol, ucontrol, 0, 0, 0);
}

static int max98925_swap_channels_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return max98925_swap_put(kcontrol, ucontrol, 0, 0, 0);
}
...
static const struct snd_kcontrol_new max98925_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Swap Channels", 0, 0, 1, 0,
		max98925_swap_channels_get, max98925_swap_channels_put),

	SOC_SINGLE_EXT_TLV("Speaker Gain", MAX98925_R02D_GAIN,
		M98925_SPK_GAIN_SHIFT, (1<<M98925_SPK_GAIN_WIDTH)-1, 0,
		max98925_left_gain_get, max98925_left_gain_put, max98925_spk_tlv),

	SOC_ENUM_EXT("Speaker Enable", max98925_amplifier_enum,
		max98925_left_en_get, max98925_left_en_put),
};

static __maybe_unused int max98925_add_widgets(struct snd_soc_codec *codec)
{
	int ret;
...
static int max98925_dai_digital_mute(struct snd_soc_dai *codec_dai, int mute)
{
	...
	if (mute) {
		if (pa_enable) {
			/* when enable speaker swap, here send cmd to set speaker mode */
			if (max98925->regmapR && enable_spk_swap) {
				param = (int *) payload;
				param[0] = orient_mode;
				param[1] = 1;
				param[2] = ALG_AUTO_MUTE;
				param[3] = ALG_ENABLE_CMD;
				afe_dsm_param_ctrl(MAX_DIR_WRITE, 32, (uint8_t *)param);
				msleep(35);
			...
			size_t count, loff_t *ppos)
{
	...
	afe_dsm_param_ctrl(MAX_DIR_READ, count, payload);
			...
			size_t count, loff_t *ppos)
{
	...
	afe_dsm_param_ctrl(MAX_DIR_WRITE, count, payload);
	mutex_unlock(&dsm_lock);
...
static ssize_t spk_left_show(struct device_driver *driver, char *buf)
{
	if (pmax98925data)
		return scnprintf(buf, PAGE_SIZE, "%d", pmax98925data->left_en?1:0);
	else
		return 0;
}

static ssize_t spk_left_store(struct device_driver *drv,
								const char *buf, size_t count)
{
	unsigned long nstate = 0;

	if (count > 0 && pmax98925data) {
		while (' ' == *buf)
			buf++;

		nstate = simple_strtoul(buf, NULL, 0);
		if (nstate > 0) {
			if (DEFAULT_SWITCH_NONEED != pmax98925data->switch_en_gpio) {
				if (OPEN_SPKR == nstate)
					gpio_direction_output(pmax98925data->switch_en_gpio, GPIO_PULL_DOWN);
				else
					gpio_direction_output(pmax98925data->switch_en_gpio, GPIO_PULL_UP);
			}
			pmax98925data->left_en = 1 << M98925_EN_SHIFT;
		} else {
			pmax98925data->left_en = 0;
		}

		regmap_write(pmax98925data->regmapL, MAX98925_R038_GLOBAL_ENABLE,
					pmax98925data->left_en);
	}
	return count;
}
DRIVER_ATTR(spk_left, 0664, spk_left_show, spk_left_store);

static ssize_t spk_right_show(struct device_driver *driver, char *buf)
{
	if (pmax98925data)
		return scnprintf(buf, PAGE_SIZE, "%d", pmax98925data->right_en?1:0);
	else
		return 0;
}

static ssize_t spk_right_store(struct device_driver *drv,
									const char *buf, size_t count)
{
	unsigned long nstate = 0;

	if (count > 0 && pmax98925data && pmax98925data->regmapR) {
		while (' ' == *buf)
			buf++;
		nstate = simple_strtoul(buf, NULL, 0);

		if (nstate > 0)
			pmax98925data->right_en = 1 << M98925_EN_SHIFT;
		else
			pmax98925data->right_en = 0;

		regmap_write(pmax98925data->regmapR, MAX98925_R038_GLOBAL_ENABLE,
					pmax98925data->right_en);
	}
	return count;
}
DRIVER_ATTR(spk_right, 0664, spk_right_show, spk_right_store);

static ssize_t recv_gpio_show(struct device_driver *driver, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", gpio_status);
}

static ssize_t recv_gpio_store(struct device_driver *drv,
									const char *buf, size_t count)
{
	unsigned long nstate = 0;

	if (count > 0 && DEFAULT_SWITCH_NONEED != pmax98925data->switch_en_gpio) {
		nstate = simple_strtoul(buf, NULL, 0);
...
DRIVER_ATTR(recv_gpio, 0664, recv_gpio_show, recv_gpio_store);

static ssize_t max_reg_show(struct device_driver *driver, char *buf)
{
	int val_l = 0;
	int val_r = 0;
	int i = 0;
	...
	strlcpy(buf, "addr L R\n", PAGE_SIZE);
	while (0 != reg_table[i].count) {
		for (j = 0; j < reg_table[i].count; j++) {
			addr = j + reg_table[i].start;
			ret = regmap_read(pmax98925data->regmapL, addr, &val_l);
			if (ret < 0) {
				pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
			}

			if (pmax98925data->regmapR) {
				ret = regmap_read(pmax98925data->regmapR, addr, &val_r);
				if (ret < 0) {
					pr_err("%s: regmap_read failed, err = %d\n", __func__, ret);
				}
			}

			snprintf(tmpbuf, 64, "%02X %02X %02X\n", addr, val_l, val_r);
			strlcat(buf, tmpbuf, PAGE_SIZE);
		}
		i++;
	}
	return strlen(buf)+1;
...
static ssize_t max_reg_store(struct device_driver *drv,
								const char *buf, size_t count)
{
	...
	if (count > 0 && pmax98925data) {
		...
		regval = simple_strtoul(start, &start, 0);
		if (MAX98925_R03A_BOOST_LIMITER < regval && MAX98925_R0FF_VERSION != regval)
			return -EFAULT;

		while (' ' == *start)
			start++;

		lval = simple_strtoul(start, &start, 0);
		rval = lval;

		regmap_write(pmax98925data->regmapL, regval, lval);
		if (pmax98925data->regmapR) {
			while (' ' == *start)
				start++;

			if (0 == kstrtoul(start, 0, &tmpval))
				rval = tmpval;

			regmap_write(pmax98925data->regmapR, regval, rval);
		}
	}

	return count;
}
DRIVER_ATTR(max_reg, 0664, max_reg_show, max_reg_store);

static struct attribute *spk_attrs[] = {
	&driver_attr_spk_left.attr,
	&driver_attr_spk_right.attr,
	&driver_attr_recv_gpio.attr,
	&driver_attr_max_reg.attr,
	NULL,
};

static struct attribute_group spk_group = {
	.attrs = spk_attrs,
};

static const struct attribute_group *maxgroups[] = {
	&spk_group,
	NULL,
};
							...
							const struct i2c_device_id *id)
{
	...
	if (NULL == max98925) {
		pr_err("malloc max98925_priv failed\n");
		return -ENOMEM;
	}
	...
	snprintf(i2c_node_info, DEBUG_BUF_LEN, "max98925.%s", dev_name(&i2c_l->dev));
	pr_info("%s, i2c node '%s'", __func__, i2c_node_info);
	...
	if (0 != paddr) {
		...
		max98925->regmapR = NULL;
		max98925_i2c_right->addr = 0;
	}
	...
	ret = of_get_named_gpio(i2c_l->dev.of_node, CONFIG_SPEAKER_SWITCH, 0);
	if (ret < 0) {
		pr_info("%s:Unable to get speaker switch gpio pin number\n", __func__);
		max98925->switch_en_gpio = DEFAULT_SWITCH_NONEED;
		ret = 0;
	} else {
		pr_info("%s: read receiver gpio  %d\n", __func__, ret);
		max98925->switch_en_gpio = ret;
		/* request var for gpio with receiver */
		ret = gpio_request(max98925->switch_en_gpio, CONFIG_SPEAKER_SWITCH);
		if (ret) {
			pr_err("%s: Failed to config spk&rcv switch gpio %u\n",
	...
	/* read speaker swap config */
	enable_spk_swap = of_property_read_bool(i2c_l->dev.of_node, ENABLE_SPEAKER_SWAP);

	pmax98925data = max98925;
	pr_debug("%s: ret %d\n", __func__, ret);

	len = 0;
	ret = regmap_read(max98925->regmapL, MAX98925_R0FF_VERSION, &len);
	if (ret < 0) {
		pr_err("%s: L get smartpa ver failed, err = %d\n", __func__, ret);
		goto err_out;
	}

	if (NULL != max98925->regmapR) {
		len = 0;
		ret = regmap_read(max98925->regmapR, MAX98925_R0FF_VERSION, &len);
		if (ret < 0) {
			pr_err("%s: R get smartpa ver failed, err = %d\n", __func__, ret);
			goto err_out;
		}
	}
err_out:
...
bool get_max_smartpa_drv_state(void)
{
	return g_max_drv_err;
}

void get_max_smartpa_i2c_node(uint8_t *info)
{
	if (NULL == info) {
		return;
	}

	strncpy(info, i2c_node_info, strlen(i2c_node_info));
	return;
}
